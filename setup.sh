...
# We need to pass userConfig to the zsh module so it can access the home directory
cat > home/shell/zsh.nix << 'EOF'
{ pkgs, config, ... }:
{
  programs.zsh = {
    enable = true;
...
echo "Zsh configuration fixed!"
echo "The key change: zplugHome now uses config.home.homeDirectory instead of \$HOME"
